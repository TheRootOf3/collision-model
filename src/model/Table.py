...
class Table:
    ...
    def __copy_table(self):
        ...
        newTable = Table(self.__dimX, self.__dimY)
        newTable.balls = copy.deepcopy(self.balls)
        newTable.__friction = self.__friction
    ...
    def __update_table(self):
        ...
        # print("abc")
        if len(self.__collision_list) != 0:
            for element in self.__collision_list:
                for ball in self.balls:
    ...
    def predict_all_collisions(self, depth_time):
        ...
        self.__collision_list = []
        pred_table = self.__copy_table()  # Create copy of the table for predictions
        ...
        while depth_time >= 0:
            ...
            if first_col[0] == 0:
                ...
                self.__collision_list.append([first_col[1].ballID, pred_table.d_time + first_col[-1], newball])
                ...
                first_col[1].vel_vector, first_col[2].vel_vector = \
                    pred_table.calculate_new_vectors(first_col[1].mass,
                                                     first_col[2].mass,
                                                     first_col[1].vel_vector,
                                                     first_col[2].vel_vector,
                                                     first_col[1].pos,
                                                     first_col[2].pos)
                # print(np.linalg.norm(first_col[1].vel_vector), np.linalg.norm(first_col[2].vel_vector))
                ...
                self.__collision_list.append([first_col[1].ballID, pred_table.d_time + first_col[-1], newball1])
                self.__collision_list.append([first_col[2].ballID, pred_table.d_time + first_col[-1], newball2])
                pred_table.d_time += first_col[-1]
    ...
    def calculate_ball_col_time(self, ball1, ball2):
        ...
        for x in roots:
            if self.check_calculated_time(x, ball1) and self.check_calculated_time(x, ball2):
                # print("yes")
                prec = 13
                while np.linalg.norm((ball1.pos + (x * ball1.vel_vector)) - (
                        ball2.pos + (x * ball2.vel_vector))) < ball1.r + ball2.r and prec > 0:
    ...
    def check_calculated_time(self, time, ball):
        ...
        if time <= 0 or isinstance(time, complex):  # added time <= 0.00001 to get rid of two balls blocking. May cause problems!
            # print("NOPE1")
            return False
        if 0 + ball.r <= ball.pos[0] + time * ball.vel_vector[0] <= self.__dimX - ball.r and \
                0 + ball.r <= ball.pos[1] + time * ball.vel_vector[1] <= self.__dimY - ball.r:
            # print("YUP")
            return True
        # print("NOPE2")
        return False
    ...
    def calculate_wall_col_time(self, ball):
        ...
        if ball.vel_vector[0] != 0:
            ...
            times[1] = (self.__dimX - ball.r - ball.pos[0]) / ball.vel_vector[0]

        if ball.vel_vector[1] != 0:
            times[2] = (0 + ball.r - ball.pos[1]) / ball.vel_vector[1]
            times[3] = (self.__dimY - ball.r - ball.pos[1]) / ball.vel_vector[1]

        col_time = 1000000
        wall = None  # wall 0 - left, wall 1 - right, wall 2 - bottom, wall 3 - top
        for i in range(len(times)):
            if col_time > times[i] > 9e-13:  # added time <= 0.00001 to get rid of two balls blocking. May cause problems!
                col_time = times[i]
                wall = i
    ...
    def generate_cords(self):
        ...
        pos = np.array([random.uniform(r, self.__dimX - r), random.uniform(r, self.__dimY - r)])
    ...
    def calculate_new_vectors(self, m1, m2, v1, v2, pos1, pos2):
        ...
        v_new1 = v1 - 2 * m2 / (m1 + m2) * (np.dot(v1 - v2, pos1 - pos2) / (np.linalg.norm(pos1 - pos2)) ** 2) * (
                pos1 - pos2)
        v_new2 = v2 - 2 * m1 / (m1 + m2) * (np.dot(v2 - v1, pos2 - pos1) / (np.linalg.norm(pos2 - pos1)) ** 2) * (
                pos2 - pos1)
